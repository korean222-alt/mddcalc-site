...
## 3-1. 배포 규칙과 데이터 갱신 (2026-08-23)

**고쳤으면 배포까지가 한 세트다.** 커밋만 하고 끝내지 않는다 — 자세한 이유와 절차는
저장소 루트의 `CLAUDE.md` 에 적어 뒀다. 요약하면:

1. 고친다 → 2. 테스트·브라우저로 검증 → 3. 커밋·푸시 → 4. **배포 성공까지 확인** → 5. 보고

`vercel.json` 에 JSON 주석(`"//"`)을 넣었다가 스키마 검증에서 막혀 배포가 통째로
실패한 적이 있다. 커밋은 멀쩡했고 사이트는 그대로였다. JSON 에 주석은 없다.

### 데이터는 언제 것이고 언제 갱신되나

실시간이 아니다. **마지막으로 마감된 거래일의 종가**다.

| 무엇 | 언제 갱신 | 워크플로 |
|---|---|---|
| `data/sectors.json` (섹터 RS·히트맵) | 평일 KST **18:30**(한국 종가) / **익일 07:30**(미국 종가) | `refresh-kr-data.yml` |
| `stock/*.html` (종목 리포트) | 매주 월 03:00 UTC | `refresh-stock-pages.yml` |

- 미국 데이터는 한국 시각 기준 **한 거래일 늦게** 반영된다.
- 주말·공휴일에는 돌지 않는다. 월요일 아침에 보는 건 금요일 종가다.
- 장중 실시간은 지원하지 않는다. 무료 소스로는 불가능하고, 하려면 유료 시세 API 와
  서버가 필요해 지금 구조(정적 파일 + Vercel)를 통째로 바꿔야 한다.
  **"실시간처럼 보이게" 만들지 말 것.** 화면에는 늘 기준일이 적혀 있고,
  4일 넘게 밀리면 경고가 뜬다(`assets/site.js` 의 `rsStaleNote()`).
